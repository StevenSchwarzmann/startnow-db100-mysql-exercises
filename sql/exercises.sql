...
select * from customer where active like 0 or last_name like '%m';
...
select staff_id, first_name, last_name, address_id, picture, email, store_id, active, username, last_update from staff where staff_id like 1;

# 3h. Select all columns minus the password column from the staff table for rows that do not contain a password.
select staff_id, first_name, last_name, address_id, picture, email, store_id, active, username, last_update from staff where staff_id like 2;
...
select title, description, special_features, length, rental_duration from film where special_features like '%Behind the Scenes%' order by length desc limit 10 ;
...
SELECT C.first_name AS customer_first_name , C.last_name AS customer_last_name, T.first_name AS actor_first_name, T.last_name As actor_last_name
FROM customer AS C
LEFT JOIN actor AS T ON C.last_name = T.last_name;
...
select C.first_name AS customer_first_name, C.last_name AS customer_last_name, T.first_name AS actor_first_name, T.last_name AS actor_last_name
from customer AS C 
RIGHT JOIN actor AS T on C.last_name = T.last_name;
...
select C.first_name AS customer_first_name, C.last_name AS customer_last_name, T.first_name AS actor_first_name, T.last_name AS actor_last_name
from customer AS C 
INNER JOIN actor AS T on C.last_name = T.last_name;
...
select A.city as city, B.country as country
from city as A
left join country AS B on a.city = b.country;
...
select title, description, release_year, F.language_id as language 
from film as F
left join language as L on F.language_id = L.name;
